-- SQL for en-til-mange-eksemplet gjennomgått i timen onsdag 18. mars 2026
-- og fullført fredag 20. mars 2026.

DROP SCHEMA IF EXISTS forelesning3_one2many CASCADE;
CREATE SCHEMA forelesning3_one2many;
SET search_path TO forelesning3_one2many;
    
CREATE TABLE vitnemal
(
	studnr INTEGER,
	studiestart DATE NOT NULL,
	studieslutt DATE,

	CONSTRAINT vitnemalPK PRIMARY KEY (studnr)
);

CREATE TABLE karakter
(
	karnr SERIAL,
	emnekode CHAR(6) NOT NULL,
	eksdato DATE NOT NULL,
	bokstav CHAR(1),
	studnr INTEGER NOT NULL,

	CONSTRAINT karakterPK PRIMARY KEY (karnr),
	CONSTRAINT karakterUnique UNIQUE (emnekode, studnr), -- Altså kun én kar per emne for en student
	CONSTRAINT vitnemalFK FOREIGN KEY (studnr) REFERENCES Vitnemal(studnr)
);

INSERT INTO
  vitnemal(studnr, studiestart, studieslutt)
VALUES
    (123456, '2021-08-15', '2024-06-15'),
    (234567, '2021-08-15', NULL), -- Studenten er ikke ferdig å studere ennå
    (345678, '2021-08-15', '2024-06-15');

INSERT INTO
  -- NB! Databasen genererer selv primærnøkler. De skal ikke oppgis ved INSERT i karakter.
  karakter(emnekode, eksdato, bokstav, studnr)
VALUES
    ('DAT107', '2022-05-30', 'A', 123456),
    ('DAT102', '2022-06-04', 'A', 123456),
    ('DAT107', '2022-05-30', 'B', 234567);

-- En liten SQL-spørring til slutt som finner DAT107-karakterer for studenter som er
-- ferdig (har sluttdato). Forventer kun (1, DAT107, '2022-05-30', 'A', 123456)
SELECT *
FROM karakter k
NATURAL JOIN vitnemal v  -- Tilsvare JOIN vitnemal v ON k.studnr = v.studnr
WHERE v.studieslutt IS NOT NULL AND k.emnekode LIKE 'DAT107';
   

    
    
    
    